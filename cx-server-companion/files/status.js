...
const ps = spawnSync('docker', ['ps', '--no-trunc', '--format', '{{ json . }}', '--filter', 'name=REPLACE-ME']);
...
if (containers.length === 0) {
    ...
    if (expectDownloadCacheIsRunning) {
        if (containers.filter(c => c.Names.includes("REPLACE-ME-nexus")).length === 0) {
    ...
    console.log(spawnSync('docker', ['ps', '--filter', 'name=REPLACE-ME']).stdout.toString())
}
